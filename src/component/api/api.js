...
export function fetchCityNow(cityname) {
  return fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cityname}&units=metric&appid=806af346335195477bb426c79a997f1f`)
        .then(res => res.json())
        .then(res => {
          if (res.cod === 200){
          let object = {};
...
export function fetchLocalNow(latitude, longitude) {
  return fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&units=metric&appid=806af346335195477bb426c79a997f1f`)
    .then(res => res.json())
    .then(res => {
      let object = {};
      object.city = res.name;        
...
export function fetchForecast(cityname, cnt) {
  return fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${cityname}&appid=806af346335195477bb426c79a997f1f&units=metric&cnt=${cnt}`)
    .then(res => res.json())
    .then(res => {
      return res;
    })
