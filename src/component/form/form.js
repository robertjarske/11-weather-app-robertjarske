...
import { fetchCityNow, fetchLocalNow, fetchForecast, getPromiseDataFromArray } from '../api/api';
...
class Form extends Component {
	...
	toFahrenheit(celsius) {
		...
		// 	celsius.map((interval, index) => {
				
		// 		let fahrenheitMax = ((interval.main.temp_max * 9 / 5) + 32).toFixed(2);
		// 		let fahrenheitMin = ((interval.main.temp_min * 9 / 5) + 32).toFixed(2);
		// 		let fahrenheit = ((interval.main.temp * 9 / 5) + 32).toFixed(2);

		// 		this.setState(({forecast, main}) => ({forecast: {
		// 			...forecast,
		// 				main: {
		// 					...main,
		// 					temp: fahrenheit,
		// 					temp_max: fahrenheitMax,
		// 					temp_min: fahrenheitMin
		// 				}
		// 		}}), function() {
		// 			const test = this.state.forecast;
		...
		// 		})
				...
				// 				temp_max: fahrenheitMax,
				// 				temp_min: fahrenheitMin,
				// 				humidity: this.state.forecast[index].main.humidity
				// 			}
				// 		}
	...
	toCelsius(fahrenheit) {
		...
		// let celsiusMin = (fahrenheit - 32) * 5 / 9;
	...
	render() {
		...
		return(
			<div>
				...
				<button onClick={this.getLocation.bind(this)}>Get the current weather for my location</button>
				{ this.state.error !== null ?
					<div className="error">{this.state.error}</div>
					: ''}
					{ this.state.city.length > 0 ?
				<div className="App-weather">
						<button onClick={this.getFiveDayForecast.bind(this)}>Get five day overview</button>
					<div>
						<label className="switch">
							<input onChange={this.onChange.bind(this)} value={this.state.isCelsius} type="checkbox"/>
					...
					<p>Latitude: {this.state.coordinates.lat} | Longitude: {this.state.coordinates.lon}</p>
					<p>Date: {this.state.dateTime.date}</p>
					<p>Sunrise: {this.state.sun.sunrise} | Sunset: {this.state.sun.sunset}</p>
					<img src={`http://openweathermap.org/img/w/${this.state.weather.icon}.png`} alt="Weather icon" />
					<p>
						{this.state.weather.description}
					</p>
					<p>{this.state.climate.temp} &deg;{this.state.unit} | Humidity: {this.state.climate.humidity}%</p>
					<h3>Wind</h3>
					<div className="compass">
						<div className="direction">
							<p className="p">{this.state.windDirection}<span className="wind">{this.state.wind.speed}m/s</span></p>
						</div>
						<div className="arrow" style={{transform: `rotate(${this.state.wind.degrees}deg)`}}></div>
					</div>
				</div>
			: <p>No results yet</p>}
				...
				<h3 className="forecast-title">Forecast for the next 24h</h3>
					{
						this.state.forecast.map((interval, index) => { 
							return <Day onChange={this.onChange.bind(this)} isCelsius={this.state.isCelsius} key={index} interval={interval} />
						})
					}
				</div>
				: ''
